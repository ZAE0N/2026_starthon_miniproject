...
# ── 2. 443 포트 ─────────────────────────────────────────────────────────
#
# ★ 이 시점에 nginx 는 아직 443 을 듣지 않습니다. https 블록은 인증서가
#   생긴 뒤 10-deploy-app.sh 가 씁니다. 그래서 "연결되나?" 만 보면
#   방화벽을 제대로 열어 둔 사람에게도 "닫혀 있다"고 말하게 됩니다.
#
#   둘은 실패 방식이 다릅니다.
#     · 방화벽이 막음      → 패킷이 버려져 응답이 없음 → 타임아웃 (124)
#     · 열렸는데 안 들음   → 커널이 RST 를 보냄 → 즉시 거부 (124 아님)
#   이 차이로 구분합니다.
say "443 포트 확인"
timeout 8 bash -c "</dev/tcp/$SERVER_IP/443" 2>/dev/null
PORT_RC=$?
if [ "$PORT_RC" = 0 ]; then
  ok "443 이 열려 있고 무언가 듣고 있습니다"
elif [ "$PORT_RC" = 124 ]; then
  warn "443 이 방화벽에 막혀 있습니다 (응답 없이 타임아웃)"
  echo "   Lightsail 콘솔에서 여세요 (80 때와 같은 자리입니다):"
  echo "     인스턴스 → [네트워킹] → [규칙 추가]"
  echo "        애플리케이션: HTTPS   프로토콜: TCP   포트: 443"
  echo "     ※ '특정 IP 주소로 제한' 은 체크하지 마세요. 심사위원이 접속해야 합니다."
  echo
  echo "   발급 자체는 80 포트로 이뤄지므로 지금 진행해도 됩니다."
  echo "   다만 10-deploy-app.sh 를 돌린 뒤 접속하려면 반드시 열려 있어야 합니다."
  confirm "계속할까요?"
else
  ok "443 이 열려 있습니다 (아직 nginx 가 듣지 않는 것은 정상입니다)"
  echo "   https 블록은 인증서를 받은 뒤 10-deploy-app.sh 가 씁니다."
fi
