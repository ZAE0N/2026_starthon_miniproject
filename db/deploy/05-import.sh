...
if [ "$CS" = "utf8mb4" ]; then
  ...
  warn "서버 기본 문자셋이 '$CS' 입니다."
  warn "이 덤프에는 CHARSET 선언이 없어서 테이블이 '$CS' 로 만들어집니다. 한글이 깨집니다."
  warn "02-install-mysql.sh 를 먼저 돌려 서버 기본값을 utf8mb4 로 고정하세요."
...
if rsh "sudo mysql -N -B -e 'SHOW DATABASES;'" | grep -qx "$DB_NAME"; then
  ...
  if ! rsh "ls ~/db/backup_${DB_NAME}_*.sql >/dev/null 2>&1"; then
    die "백업이 없습니다. 먼저 ./04-backup.sh 를 돌리세요"
  fi
